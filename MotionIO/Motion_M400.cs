    ...
    public class SctCardCollect
    {
        ...
        public SctCardCollect()
        {
            try
            {
                ...
                {
                    ErrorCode err = (ErrorCode)SctBase.SCT_Initial(nCardNo);
                    if (err != ErrorCode.Success)
                        break;

                    int nCardType = 0;
                    err = (ErrorCode)SctBase.SCT_GetCardType(nCardNo, ref nCardType);
                    if (err != ErrorCode.Success)
                        break;
            ...
            {
                System.Windows.Forms.MessageBox.Show(e.StackTrace, "SCT控制卡初始化失败", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    ...
    public class Motion_M400 : Motion
    {
        ...
        public override bool Init()
        {
            ...
            try
            {
                ...
                {
                    ErrorCode err = (ErrorCode)SctBase.SCT_LoadConfigFile("M400Cfg.DAT", m_nInternalIndex);
                    if (err == ErrorCode.Success)
                    {
                        m_bEnable = true;
                        return true;
        ...
        {
            ErrorCode err = (ErrorCode)SctBase.SCT_Initial(m_nInternalIndex);
            if (err == ErrorCode.Success)
            {
                return true;
            }
        ...
        {
            ErrorCode err = (ErrorCode)SctBase.SCT_SetServoOn(m_nInternalIndex, nAxisNo, 1);
            if (err == ErrorCode.Success)
            {
                return true;
            }
        ...
        {
            ErrorCode err = (ErrorCode)SctBase.SCT_SetServoOn(m_nInternalIndex, nAxisNo, 0);
            if (err == ErrorCode.Success)
            {
                return true;
            }
        ...
        {
            int nEnable = 0;
            ErrorCode err = (ErrorCode)SctBase.SCT_GetServoOnState(m_nInternalIndex, nAxisNo, ref nEnable);
            if (err != ErrorCode.Success)
            {
                if (m_bEnable)
        ...
        {          
            ErrorCode err = (ErrorCode)SctBase.SCT_HomeStart(m_nInternalIndex, nAxisNo);
            if (err == ErrorCode.Success)
            {
                return true;
            }
        ...
        private ErrorCode SetSpeed(int nAxisNo, int nSpeed)
        {
            ...
            do
            {
                ...
                err = (ErrorCode)SctBase.SCT_SetSpeed(m_nInternalIndex, nAxisNo, nSpeed);
                if (err == ErrorCode.Success)
        ...
        public override bool AbsMove(int nAxisNo, int nPos, int nSpeed)
        {
            ...
            err = (ErrorCode)SctBase.SCT_AbsoluteMove(m_nInternalIndex, nAxisNo, nPos);
            if (err == ErrorCode.Success)
            {
                return true;
            }
        ...
        public override bool RelativeMove(int nAxisNo, int nPos, int nSpeed)
        {
            ...
            err = (ErrorCode)SctBase.SCT_RelativeMove(m_nInternalIndex, nAxisNo, nPos);
            if (err == ErrorCode.Success)
            {
                return true;
            }
        ...
        public override bool JogMove(int axis, bool bPositive, int bStart, int nSpeed)
        {
            ...
            ErrorCode err = (ErrorCode)SctBase.SCT_JogMove(m_nInternalIndex, axis, nDir);
            if (err == ErrorCode.Success)
            {
                return true;
            }
        ...
        {
            ErrorCode err = (ErrorCode)SctBase.SCT_DecStop(m_nInternalIndex, nAxisNo);
            if (err == ErrorCode.Success)
            {
                return true;
            }
        ...
        {
            ErrorCode err = (ErrorCode)SctBase.SCT_SuddenStop(m_nInternalIndex, nAxisNo);
            if (err == ErrorCode.Success)
            {
                return true;
            }
        ...
        {
            int nIsMoving = 0;
            ErrorCode err = (ErrorCode)SctBase.SCT_IsMoving(m_nInternalIndex, nAxisNo, ref nIsMoving);
            if (err == ErrorCode.Success)
            {
        ...
        {
            int nMotionState = 0;
            ErrorCode err = (ErrorCode)SctBase.SCT_GetMotionState(m_nInternalIndex, nAxisNo, ref nMotionState);
            if (err != ErrorCode.Success)
                return -1;
        ...
        {
            int nPos = 0;
            ErrorCode Err = (ErrorCode)SctBase.SCT_GetCommandPos(m_nInternalIndex, nAxisNo, ref nPos);
            if (Err == ErrorCode.Success)
            {
                return nPos;
            }
        ...
        public override int IsAxisNormalStop(int nAxisNo)
        {
            ...
            int nStopCode = 0;
            err = (ErrorCode)SctBase.SCT_GetStopData(m_nInternalIndex, nAxisNo, ref nStopCode);
            if (err == ErrorCode.Success)
            {
        ...
        public override int IsAxisInPos(int nAxisNo)
        {
            ...
            if(nRet == 0)
            {
                int nTargetPos = 0;
                int nCommandPos = 0;
                SctBase.SCT_GetTargetPos(m_nInternalIndex, nAxisNo, ref nTargetPos);
                SctBase.SCT_GetCommandPos(m_nInternalIndex, nAxisNo, ref nCommandPos);

                if (Math.Abs(nCommandPos - nTargetPos) > 50000)
                    return 6;  //轴停止后位置超限
            }
            return nRet;
        }
        ...
        {
            ErrorCode err = (ErrorCode)SctBase.SCT_SetCommandPos(m_nInternalIndex, nAxisNo, 0);
            if (err != ErrorCode.Success)
                return false;

            err = (ErrorCode)SctBase.SCT_SetRealPos(m_nInternalIndex, nAxisNo, 0);
            if (err != ErrorCode.Success)
                return false;
        ...
        public override bool VelocityMove(int nAxisNo, int nSpeed)
        {
            ...
            ErrorCode err = (ErrorCode)SctBase.SCT_JogMove(m_nInternalIndex, nAxisNo, nDir);
            if (err == ErrorCode.Success)
            {
                return true;
            }
        ...
        public override int IsHomeNormalStop(int nAxisNo)
        {
            int nHomeFinished = 0;
            int nHomeRunning = 0;
            ErrorCode err = (ErrorCode)SctBase.SCT_GetHomeState(m_nInternalIndex, nAxisNo, ref nHomeRunning, ref nHomeFinished);
            if (err != ErrorCode.Success)
                return -1;
