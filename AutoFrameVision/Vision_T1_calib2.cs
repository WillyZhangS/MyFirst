...
namespace AutoFrameVision
{
    ...
    /// 视觉处理步骤样板类2
    /// </summary>
    public class Vision_T1_calib2:VisionBase
    {
        ...
        public Vision_T1_calib2(string strName):base(strName)
        {
        }
        ...
        public override bool LoadParam()
        {
            ...
            string strTemp = IniOperation.GetStringValue(strFile, "T1_calib2", "ExposureTime", null);
        ...
        public override bool SaveParam()
        {
            ...
            IniOperation.WriteValue(strFile, "T1_calib2", "ExposureTime", m_ExposureTime.ToString());
        ...
        public override bool ProcessImage(VisionControl vc)
        {
            ...
            try
            {
                 hde.T1_calib2(imgSrc, m_strDir, ModelId, ModelData, out data);
                if (data[0] == 1)
                {
                    SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1Calib2_X, data[1], false);
                    SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1Calib2_Y, data[2], false);
                    return true;
                }
                else
                {
                    SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1Calib2_X, data[1], false);
                    SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1Calib2_Y, data[2], false);
                  //  SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1Calib2_A, data[0], true);
                    return false;
                }
            }
            catch (Exception e)
            {
                ...
                SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1Calib2_X, VisionException, false);
                SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1Calib2_Y, VisionException, false);
                SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1Calib2_A, VisionException, true);
                return false;
            }
            finally
            {
