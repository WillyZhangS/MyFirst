...
namespace AutoFrameVision
{
    ...
    /// 视觉处理步骤样板类
    /// </summary>
    public class Vision_T1_13:VisionBase
    {
        ...
        public Vision_T1_13(string strName):base(strName)
        {
        ...
        public override bool LoadParam()
        {
            ...
            string strTemp = IniOperation.GetStringValue(strFile, "T1_13", "ExposureTime", null);
        ...
        public override bool SaveParam()
        {
            ...
            IniOperation.WriteValue(strFile, "T1_13", "ExposureTime", m_ExposureTime.ToString());
        ...
        {
            double x1 = SystemMgr.GetInstance().GetRegDouble((int)SysFloatReg.T1_13_X);
            double y1 = SystemMgr.GetInstance().GetRegDouble((int)SysFloatReg.T1_13_Y);

            double x1Axis, y1Axis;
            m_caliTrans.Translate(x1, y1, out x1Axis, out y1Axis);

            //TODO:注意行列与机器人X, Y的关系
            SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1_13_X, x1Axis, true);
            SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1_13_Y, y1Axis, true);
        ...
        public override bool ProcessImage( VisionControl vc)
        {
            ...
            {
                HTuple data=0;
                hde.T1_13(imgSrc, m_strDir, ModelId, ModelData, out data);
                if (data[0] == 1)
                {
                    SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1_13_X, data[1], true);
                    SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1_13_Y, data[2], true);
                    SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1_13_A, data[3], false);
                ...
                else
                {
                    ...
                    SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1_13_X, data[0], true);
                    SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1_13_Y, data[0], true);
                    SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1_13_A, data[0], false);
                    return false;
                }
            }
            catch (Exception e)
            {
                ...
                //处理失败时，必须将无效数值写入数据区，防止使用上一次的数据
                SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1_13_X, VisionException, true);
                SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1_13_Y, VisionException, true);
                SystemMgr.GetInstance().WriteRegDouble((int)SysFloatReg.T1_13_A, VisionException, false);
                return false;
            }
            finally
            {
