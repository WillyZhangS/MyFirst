...
namespace AutoFrame
{
    ...
    /// 工位2机器人站
    /// </summary>
    class StationRobot2 : StationBase
    {
        private TcpLink m_tcpRobotCtl;
        private TcpLink m_tcpRobotComm;
        private bool bFlagFirstRun = false;

        public StationRobot2(string strName):base(strName)
        {
            io_in = new string[] { "1.6", "1.8" };
            io_out = new string[] { };
            bPositiveMove = new bool[] { true, true, true, true };
            strAxisName = new string[] { "X轴", "Y轴", "Z轴", "U轴" };
            m_tcpRobotCtl = TcpMgr.GetInstance().GetTcpLink(2);
            m_tcpRobotComm = TcpMgr.GetInstance().GetTcpLink(3);
        }
       ...
       //当所有站位均为全自动运行模式时，不需要重载该函数
        ...
        {
            ShowLog(string.Format("{0}系统位寄存器{1},写入有效值{2}", Name, (int)SysBitReg.机器人2初始化完成, false.ToString()));
            SystemMgr.GetInstance().WriteRegBit((int)SysBitReg.机器人2初始化完成, false);
            SystemMgr.GetInstance().WriteRegBit((int)SysBitReg.机器人2抛料, false);
        }
        ...
        public override void EmgStop()
        {
            if (m_tcpRobotCtl.IsOpen())
            {
                ...
                //m_tcpRobotCtl.WriteLine("BR");
            }
        }//站位急停

        public override void StationDeinit()
        {
            ShowLog("关闭机器人2控制和通讯网口");
            if (m_tcpRobotCtl.IsOpen())
            {
        ...
        private bool RobotInit()
        {
            ...
            if (m_tcpRobotCtl.IsOpen())
            {
                ...
                WaitTimeDelay(700);
                m_tcpRobotCtl.WriteLine("SL,robot2");
                WaitTimeDelay(700);
                ...
                WaitTimeDelay(700);
                m_tcpRobotCtl.ReadData(bytes, 1024);
                System.Diagnostics.Debug.WriteLine(bytes.ToString());
        ...
        public override void StationInit()
        {
            ...
            {
                throw new StationException("机器人2初始化启动失败");
            }
            ShowLog(string.Format("{0}系统位寄存器{1},写入有效值{2}", Name, (int)SysBitReg.机器人2初始化完成, true.ToString()));
            SystemMgr.GetInstance().WriteRegBit((int)SysBitReg.机器人2初始化完成, true); //机器人已经初始化完成
            ShowLog("初始化完成");
            bFlagFirstRun = true;
            //SystemMgr.GetInstance().WriteRegBit((int)SysBitReg.机器人2抛料, false);
        }
        ...
        public void CaliProcess()
        {
            ...
            while (true)
            {
                ...
                if (strCalib[i].Length > 0)
                {
                    ...
                    if (VisionMgr.GetInstance().ProcessStep("T3_calib"))  //拍照
                    {
                        fpix_x[i] = SystemMgr.GetInstance().GetRegDouble((int)SysFloatReg.T32_X);
                        fpix_y[i] = SystemMgr.GetInstance().GetRegDouble((int)SysFloatReg.T32_Y);
            //            fpix_a[i] = SystemMgr.GetInstance().GetRegDouble((int)SysFloatReg.T32_A);
                        i++;//拍到一次增加一次
                        ...
                        if (i == 11)
                        {
                            ...
                            {
                                caliTrans.SaveCaliData(VisionMgr.GetInstance().ConfigDir + "T3_2\\Calib.ini");
                                MessageBox.Show("机器人2标定完成!");
                            }
                            else
                            {
                                MessageBox.Show("机器人2标定失败, 请检查确认流程是否正确!");
                            }
                            break;
                        }
                    }
                    ...
                    {
                        if (DialogResult.Yes == ShowMessage("机器人2标定图像处理失败，是否重试"))
                        //if (DialogResult.Yes == MessageBox.Show("机器人2标定图像处理失败，是否重试", "提示信息",
        ...
        public override void StationProcess()
        {
           ...
           // WaitBegin();
            ...
            else
            {
                if (bFlagFirstRun)
                {
                    WaitRegBit((int)SysBitReg.Tray2盘XYHome完成, true); //第一次的时候等待XY Home完成
                    bFlagFirstRun = false;
                    SystemMgr.GetInstance().WriteRegBit((int)SysBitReg.Tray2盘XYHome完成, false); //清掉Tray2 XY轴home
                }
                ...
                while (true)
                {
                    ...
                    IoMgr.GetInstance().WriteIoBit(1, 13, false);//关闭fpc翻转真空2
                    WaitIo(1, 6, true, -1, false, true);//等待机器人2取料OK
                    wait_recevie_cmd(m_tcpRobotComm, "ROBOT_JUMP_PHOTO/OK");//机器人到拍照点
                    while (true)
                    {
                        if (VisionMgr.GetInstance().ProcessStep("T3_2")
                            || SystemMgr.GetInstance().Mode == SystemMode.Dry_Run_Mode) //空跑模式屏蔽拍照
                        {
                            SystemMgr.GetInstance().WriteRegBit((int)SysBitReg.机器人2抛料, false);
                            double t3x = SystemMgr.GetInstance().GetRegDouble((int)SysFloatReg.T3_Delta_X);
                            double t3y = SystemMgr.GetInstance().GetRegDouble((int)SysFloatReg.T3_Delta_Y);
                            double t3a = SystemMgr.GetInstance().GetRegDouble((int)SysFloatReg.T3_Delta_A);
                            ...
                            if (Math.Abs(t3x) < 0.5 && Math.Abs(t3y) < 0.5 && Math.Abs(t3a) < 0.3)
                            {
                                string str = string.Format("6,{0:0.000},{1:0.000},{2:0.000}", t3x, t3y, t3a);
                                m_tcpRobotComm.WriteLine(str);
                                System.Diagnostics.Debug.WriteLine(str);
                            }
                            else
                            {
                                string str = string.Format("7,{0:0.000},{1:0.000},{2:0.000}", t3x, t3y, t3a);
                                m_tcpRobotComm.WriteLine(str);
                                System.Diagnostics.Debug.WriteLine(str);
                            }
                            SystemMgr.GetInstance().WriteRegInt((int)SysIntReg.机器人2组装数, 1, true); //机器人2组装数加1
                            WaitIo(1, 8, true, 50000, false, true); //等待机器人组装完成
                            cnts = 0;
                            break;
                        }
                        else 
                        {
                            //if (DialogResult.Yes == ShowMessage("T3_2拍照失败，是否继续？"))
                            //{
                            ...
                            if (cnts >= 3)
                            {
                                ...
                                if (DialogResult.Yes == ShowMessage("机器人2连续抛料超过3次，请检查！\n继续抓取点击<继续运行>？放弃此次组装点击<退出运行>？"))
                                {
                                    SystemMgr.GetInstance().WriteRegBit((int)SysBitReg.机器人2抛料, true);
                                }
                                else
                                {
                                    SystemMgr.GetInstance().WriteRegBit((int)SysBitReg.机器人2抛料, false);
                            ...
                            SystemMgr.GetInstance().WriteRegInt((int)SysIntReg.机器人2抛料数, 1, true);  //机器人2抛料数加1 
                            WaitIo(1, 8, true, 50000, false, true); //等待机器人组装完成
                            break;
                        }
