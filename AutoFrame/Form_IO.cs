        ...
        void UpdateIoInText(int nCardNo, int nIndex)
        {
            if (IoMgr.GetInstance().CountCard > nCardNo) //要跳转到的卡号小于总IO卡数
            {
                ...
                for ( int i = 0; i < btns_in.Length; ++i)
                {
                    ...
                    {
                        btns_in[i].Text = string.Format("{0}.{1,2} {2}", (nCardNo + 1), ( i + nIndex + 1), Card.m_strArrayIn[nIndex + i]); 
                ...
                IoCtrl iocard = IoMgr.GetInstance().m_listCard.ElementAt(m_nCardIn);
                groupBox_in.Text = string.Format("IO输入:板卡{0},序号{1},输入点{2}个", iocard.m_strCardName, m_nCardIn+1, iocard.CountIoIn);
            }
        }
        ...
        void UpdateIoOutText(int nCardNo, int nIndex)
        {
            if (IoMgr.GetInstance().CountCard > nCardNo)
            {
                ...
                for (int i = 0; i < btns_out.Length; ++i)
                {
                    ...
                    {
                        btns_out[i].Text = string.Format("{0}.{1,2} {2}", (nCardNo + 1), (i + nIndex + 1), Card.m_strArrayOut[i + nIndex]);
                ...
                IoCtrl iocard = IoMgr.GetInstance().m_listCard.ElementAt(m_nCardOut);
                groupBox_out.Text = string.Format("IO输出:板卡{0},序号{1},输出点{2}个", iocard.m_strCardName, m_nCardOut+1, iocard.CountIoOut);
            }
        }
