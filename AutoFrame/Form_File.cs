...
namespace AutoFrame
{
    public partial class Form_File : Form
    {
        ...
        public void Release() { m_nMutex = 0; }
        ...
        private void button_next_Click(object sender, EventArgs e)
        {
            lock (syslock)
            {
                ...
                if (!IsRelease())
                {
                    WarningMgr.GetInstance().Info("button_next_Click并返回");
                    return;
                }
        ...
        private void button_prev_Click(object sender, EventArgs e)
        {
            lock (syslock)
            {
                ...
                if (!IsRelease())
                {
                    WarningMgr.GetInstance().Info("button_prev_Click并返回");
                    return;
                }
        ...
        delegate void dgvDelegate(List<String[]> listData, int nStart, int nEnd);
        ...
        private void SetDgvDataSource(List<String[]> listData, int nStart, int nEnd)
        {
            if (dataGridView1.InvokeRequired)
            {
                dataGridView1.BeginInvoke(new dgvDelegate(SetDgvDataSource), new object[] { listData, nStart, nEnd });
            }
            else
            {
                //禁止pnl重绘  
                //SendMessage(dataGridView1.Handle, WM_SETREDRAW, 0, IntPtr.Zero);
                if (nEnd > m_listData.Count)
                    nEnd = m_listData.Count;
                for (int i = nStart; i < nEnd; i++)
                {
                    ...
                    string[] data = listData.ElementAt(i);
                    if (data.Length > 4)
                    {
                        StringBuilder sb = new StringBuilder(data[3]);
                        for (int k = 4; k < data.Length; ++k)
        ...
        private void ThreadLoadData(string strPath)
        {
            ...
            try
            {
                ...
                while (m_bRunThread)
                {
                    ...
                    if (IsEpual())  //到这一步代表出现BUG
                    {
                        WarningMgr.GetInstance().Info("Form_File线程出现BUG");
                        MessageBox.Show("Form_File线程出现BUG");
                        m_bRunThread = false;
                    }
                }
            }
