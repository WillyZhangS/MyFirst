        ...
        private void AxisMove(int nIndex, bool bPositive )
        {
            ...
            if (sta != null)
            {
                ...
                if (nAxisNo > 0)
                {
                    ...
                    {
                        ManaulTool.absMove(nAxisNo, textBox_tar[nIndex], textBox_speed[nIndex], bPositive && sta.bPositiveMove[nIndex]);
                    }
                    if (radioButton_rel.Checked)
                    {
                        ManaulTool.relMove(nAxisNo, textBox_tar[nIndex], textBox_speed[nIndex], bPositive && sta.bPositiveMove[nIndex]);
                    }
                    if (radioButton_jog.Checked)
                    {
                        ManaulTool.jogMove(nAxisNo, textBox_tar[nIndex], textBox_speed[nIndex], bPositive && sta.bPositiveMove[nIndex]);
                    }
                }
            }        
        }
